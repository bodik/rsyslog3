...
class iptables (
	...
	package { ["netfilter-persistent", "iptables-persistent"]: ensure => absent, }
	...
	file { "/var/lib/iptables/active":
		source => [
			$rules_v4,
			"puppet:///modules/${module_name}/PRIVATEFILE_rules.v4.${fqdn}",
			"puppet:///modules/${module_name}/rules.v4.${fqdn}",
			"puppet:///modules/${module_name}/rules.v4"],
	...
	file { "/var/lib/ip6tables/active":
		source => [
			$rules_v6,
			"puppet:///modules/${module_name}/PRIVATEFILE_rules.v6.${fqdn}",
			"puppet:///modules/${module_name}/rules.v6.${fqdn}",
			"puppet:///modules/${module_name}/rules.v6"],
		owner => "root", group => "root", mode => "0640",
